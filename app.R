...
# Load helper objects and mappings
source("helpers.R")

# Load sample data
my_sample <- readRDS("my_sample_temp.rds")

# Define UI for correlation exploration app
ui <- fluidPage(
  # Title Panel
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # Radio buttons for household language, SNAP recipient, and education level
      ...
      sliderInput("corr_n", "Sample Size", min = 20, max = 500, value = 50),
    ...
    mainPanel(
      ...
      # Conditional panel for correlation guessing game
      conditionalPanel(
...
server <- function(input, output, session) {
  ...
  # Create reactiveValues to store sampled data and true correlation
  sample_corr <- reactiveValues(corr_data = NULL, corr_truth = NULL)
  
  # Ensure X and Y variable selections are not identical
  ...
  # Generate sample data upon button click
  observeEvent(input$corr_sample, {
    
    # Subset by household language
    ...
    # Subset by educational attainment
    if (input$schl_corr == "all") {
      schl_sub <- SCHLvals
    } else if (input$schl_corr == "no_hs") {
      schl_sub <- SCHLvals[c("0", "01", "02", "03", "04", "05", "06", "07", 
  ...
  # Correlation guessing game logic (unchanged)
